...
namespace Utils;
/**
 * Pinyin Processing Class File
 *
 * @package   library.platform.gadget.pinyin
...
/**
 * Pinyin Processing Class
 */
class Pinyin
{

    /**
     * PinYin Dictionary: Pinyin - GB2312
     */
    static $arrPinYinDictionary = array(
        array('a'      , -20319),
        array('ai'     , -20317),
        array('an'     , -20304),
        array('ang'    , -20295),
    ...
    static function getPinYinByGb2312($intCode, $upCase = false)
    {
        ...
        if ($intCode > 0 && $intCode < 160) {
            return chr($intCode);
        // If it's a valid gb2312 code, do convert
        } elseif ($intCode > -20320 && $intCode < -10246) {
            for ($i = count(self::$arrPinYinDictionary) - 1; $i > -1; $i--) {
                if (self::$arrPinYinDictionary[$i][1] <= $intCode) {
                    return $upCase
                         ? ucwords(self::$arrPinYinDictionary[$i][0])
                         : self::$arrPinYinDictionary[$i][0];
    ...
    /**
     * Walk around the gb2312 string and return it's pinyin
     *
     * @param  string  $str
     * @param  boolean $upCase
     * @return string
     */
    static function rawConvByGb2312($str, $upCase = false)
    {
        $result = '';
        $length = strlen($str);
        for ($i = 0; $i < $length; $i++) {
    ...
    /**
     * Conv the string to gb2312 and return it's pinyin
     *
     * @param  string  $str
     * @param  boolean $upCase
     * @return string
    ...
    static function conv($str, $minLen = 0, $maxLen = 0, $upCase = 0, $dreg = 0)
    {
        ...
        $str = is_array($dreg)
             ? preg_replace('/' . implode('|', $dreg) . '/', '', $str)
             : $str;
        // Do rawConvByUtf8, then catch only Roman characters and numbers
        $str = preg_replace(
            '/^[^0-9A-Za-z]|[^0-9A-Za-z-]*|[^0-9A-Za-z]$/x',
            '',
            self::rawConvByUtf8($str, $upCase)
        );
    ...
    static function lowerCase($str, $minLen = 0, $maxLen = 0, $upCase = 0, $dreg = 0)
    {
        ...
        $str = preg_replace(
            '/^[^0-9A-Za-z]|[^0-9A-Za-z-]*|[^0-9A-Za-z]$/x',
            '',
            strtolower(self::rawConvByUtf8($str, $upCase))
        );
        $len = strlen($str);
        return ($minLen && $len < $minLen)
             ? ($str . substr($md5, 0, $minLen - $len))
